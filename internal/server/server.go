...
import (
	...
	"os"
	"time"
	"videochat-system/internal/handlers"
...
func Run() error {
	...
	app.Get("/stream/:suuid/websocket", websocket.New(handlers.StreamWebsocket, websocket.Config{HandshakeTimeout: 10 * time.Second}))
	app.Get("/stream/:suuid/chat/websocket", websocket.New(handlers.StreamChatWebsocket))
	app.Get("/stream/:suuid/viewer/websocket", websocket.New(handlers.SteamViewerWebSocket))
	app.Static("/", "./assets")
